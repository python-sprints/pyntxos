...
def get_bilbao_turismo_restaurant_infos(restaurant_links):
    ...
    for index, restaurant_link in enumerate(restaurant_links):
        ...
        address = dict(
            ...
            telephone=str(address.find('span', itemprop='telephone').contents[0]).strip(),
        )
...
def get_longitude_latitude_info(data):
    ...
    for index, pintxo in enumerate(data):
        ...
        if location is not None:
            print(f'{index} - Success - {pintxo["name"]} - Getting Longitude and Latitude')
            data[index]['geopy_address'] = location.address
            data[index]['longitude'] = location.longitude
            data[index]['latitude'] = location.latitude
        else:
            print(f'{index} - Failure - {pintxo["name"]} - Getting Longitude and Latitude')
            failure_count += 1

    print(f'{failure_count / len(data) * 100:.0f}% failed for getting longitude and latitude.')
    return data
